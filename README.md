...
#include "smoke.h"     // ✔️ CORRECT
// WiFi credentials
const char* ssid = "niku";
const char* password = "niku4912";

// Create AsyncWebServer on port 80
AsyncWebServer server(80);

// Create Dashboard instance
ESPDashboardPlus dashboard("IOT DASHBOARD");
...
void setup() {
    ...
    dashboard.setTitle("IOT DASHBOARD", "Home Automation Hub");
   ...
   // dashboard.addGroup("controls", "Device Controls");
    ...
    dashboard.addGroup("smoke", "Smoke Monitor");

    dashboard.addStatCard("gasValue", "Gas Value", "0", "");
    dashboard.addStatCard("smokeCount", "Smoke Count", "0", "");

    StatusCard* smokeStatus = dashboard.addStatusCard("smokeStatus", "Smoke Status", StatusIcon::WARNING);
    smokeStatus->setStatus(StatusIcon::CHECK, CardVariant::SUCCESS, "No Smoke", "");
    StatusCard* buzzerStatus = dashboard.addStatusCard("buzzerStatus", "Buzzer Status", StatusIcon::WARNING);
    buzzerStatus->setStatus(StatusIcon::CHECK, CardVariant::SUCCESS, "OFF", "");
    dashboard.addCardToGroup("smoke", "buzzerStatus");
    dashboard.addCardToGroup("smoke", "gasValue");
    dashboard.addCardToGroup("smoke", "smokeCount");
    dashboard.addCardToGroup("smoke", "smokeStatus");
    // ========================================
...
void loop() {
    ...
    // 📊 Update Smoke Data
    // ===============================
    dashboard.updateStatCard("gasValue", String(getGasValue()));
    dashboard.updateStatCard("smokeCount", String(getSmokeCount()));

    if (isSmokeDetected()) {

        // 🔥 Smoke Status
        dashboard.updateStatusCard("smokeStatus",
            StatusIcon::WARNING,
            CardVariant::DANGER,
            "SMOKE!",
            "Alert");
        ...
        // 🔊 Buzzer ON
        dashboard.updateStatusCard("buzzerStatus",
            StatusIcon::WARNING,
            CardVariant::DANGER,
            "ON",
            "Buzzer Active");
    ...
    if (millis() - lastUpdate > 2000) {
        ...
        dashboard.updateChartCard("multi-chart", 0, temperature);
        dashboard.updateChartCard("multi-chart", 1, humidity);

        dashboard.updateChartCard("cpu-chart", cpuUsage);

        dashboard.updateChartCard("bar-chart", 0, random(20, 60));
        dashboard.updateChartCard("bar-chart", 1, random(10, 40));
    }
